...
class mochila:
    ...
    def generaciones(self):
        ...
        pob_nueva, datos, fitneestotal = self.poda(pob_nueva)
        self.pob = pob_nueva[:self.pob_max]
        return peores_individuos, mejores_individuos, medias, datos, fitneestotal
    ...
    def poda(self,pob_nueva):
        ...
        for i in lista:
            ...
            obtenido.append(self.fitness_pro(i))
            for x in i:
                count1 += 1
                if count1 < 30:
                    ...
                    datos.append([self.alimentos[x].nombre,self.alimentos[x].categoria, self.alimentos[x].energia,self.alimentos[x].proteina,self.alimentos[x].grasa,self.alimentos[x].calcio,self.alimentos[x].hierro,self.alimentos[x].vitamina_a,self.alimentos[x].tiamina,self.alimentos[x].riboflavina,self.alimentos[x].niacina,self.alimentos[x].folato,self.alimentos[x].vitamina_c,self.objetivo])
...
def inicializar():
    ...
    peores_individuos, mejores_individuos, medias, datos, fitnesstotal=mi_mochila.generaciones()
    graficar(peores_individuos, mejores_individuos, medias)
    mostrar_tabla_alimentos(datos,fitnesstotal)
...
def mostrar_tabla_alimentos(datos, fTotal):
    ventana = tk.Tk()
    ventana.title("Tabla de Alimentos")

    tabla = ttk.Treeview(ventana)
    tabla["columns"] = ["Nombre", "Categoría", "Energía", "Proteína", "Grasa", "Calcio", "Hierro", "Vitamina A", "Tiamina", "Riboflavina", "Niacina", "Folato", "Vitamina C", "Objetivo", "Resultado", "Diferencia"]
    ...
    tabla.heading("Diferencia", text="Diferencia")

    tabla.grid(row=0, column=0)

    for i, dato in enumerate(datos, start=1):
        if len(dato) >= 13:
            resultado = fTotal[i-1]  # Obtener el resultado correspondiente al índice i-1
            diferencia = dato[13] - resultado
            tabla.insert("", "end", text=str(i), values=dato + [resultado, diferencia])
        else:
